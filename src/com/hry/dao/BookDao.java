...
public class BookDao {
	...
	public ArrayList<HistoryBean> get_HistoryListInfo3(){
		ArrayList<HistoryBean> tag_Array = new ArrayList<HistoryBean>();
		Connection conn = DbUtil.getConn();
	 ...
	 *
	 */
	public ArrayList<HistoryBean> getBidListBlog(int bid){
		ArrayList<HistoryBean> tag_ArrayBlog = new ArrayList<HistoryBean>();
		Connection conn = DbUtil.getConn();
		String sql = "select * from history where bid = '" + bid+"'";
		PreparedStatement stm = null;
		ResultSet rs =null;
		try {
			stm = conn.prepareStatement(sql); //执行sql语句
			rs = stm.executeQuery(); //存取结果集
			while(rs.next()){
				HistoryBean tag = new HistoryBean();
				tag.setHid(rs.getInt("hid"));
				tag.setAid(rs.getInt("aid"));
	...
	/**
	 * 用户查找图书，根据输入的名称，精确查询到相关图书的借阅日志，然后返回一个ArrayList数组类型
	 */
	public ArrayList<HistoryBean> getLikeListBlog(String bookname){
		ArrayList<HistoryBean> tag_ArrayBlog = new ArrayList<HistoryBean>();
		Connection conn = DbUtil.getConn();
		String sql = "select * from history where bookname = '" + bookname+"'";
		PreparedStatement stm = null;
		ResultSet rs =null;
		try {
			stm = conn.prepareStatement(sql); //执行sql语句
